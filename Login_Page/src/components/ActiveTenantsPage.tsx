...
export function ActiveTenantsPage({ 
  ...
  // Filter active tenants for this apartment
  // Note: apartment._id comes from MongoDB, apartment.id is the React component prop
  const activeTenants = hasUnitsConfigured ? tenants.filter((t) => {
    const tenantApartmentId = (t.apartment as any)?._id || (t.apartment as any)?.id;
    const selectApartmentId = (apartment as any)._id || apartment.id;
    return t.status === "active" && tenantApartmentId === selectApartmentId;
  }) : [];
  console.log('[ActiveTenantsPage] Rendering with tenants:', tenants);
  console.log('[ActiveTenantsPage] Apartment ID:', (apartment as any)._id || apartment.id);
  console.log('[ActiveTenantsPage] Active tenants for apartment:', activeTenants);
